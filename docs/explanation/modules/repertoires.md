...
(`libs/analysis`, with evaluations supplied by `libs/infra/engine`, imported
as `@velachess/infra-engine`) and does not touch `libs/repertoires` at all.

The tree/index/judgment/adherence reasoning below lives at the
`libs/repertoires` module root (`tree.ts`, `position-index.ts`,
`repertoire.ts`, `judgment.ts`, `adherence.ts`) as module-level pure
policies — no DB/queue dependency of their own, imported directly by any
slice that needs them, in or out of this module. Chapter dispatch
(`findDeviation`, `judgeAgainstChapters`) lives in `libs/games/judge-games`
instead, since matching a played game against a repertoire is that
module's own behavior, not a pure policy of this one — see that module's
`dispatch.ts`/`deviation.ts`.
...
Lives in `libs/games/judge-games` (`dispatch.ts`, `deviation.ts`), not this
module — matching a played game against chapters is `judge-games`'s own
behavior, built on the tree/index this module exposes as pure policy.
...
Module root (pure policies, `libs/repertoires/`):
...
index.ts            public surface — see this module's own AGENTS.md
```

Elsewhere, redistributed by ownership (migration phase 9):

```
libs/games/judge-games/dispatch.ts     ChapterJudgment, judgeAgainstChapters
libs/games/judge-games/deviation.ts    DeviationEvent, DeviationResult, findDeviation
libs/insights/get-insights/adherence-finding.ts  AdherenceFinding, adherenceFinding
libs/drills/seed-exercises/decision-positions.ts  DecisionPosition, decisionPositionsOf
libs/repertoires/get-chapter/chapter-view.ts      chapter tree shaped for the screen
libs/repertoires/extract-repertoire/extract.ts    extractRepertoireLines, openingNameFrom
```
...
lives in `extract-repertoire.ts`, this module's own slice. Honest limits: the per-game name is the opening the
