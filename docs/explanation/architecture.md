...
vertical, including `image-loader.ts`, and `locales/` owns the Lingui
catalog and the `i18n.ts` instance every vertical activates from. No
`shared/` here either. It imports no application or chess behavior.
