...
import {
  createUser,
  listGamesForInsights,
  schema,
  upsertTrackedAccount,
} from "@velachess/infra-db";
import type { StoredGradedPly } from "@velachess/infra-db";
import { createLoopHarness, type LoopHarness } from "@velachess/test-utils";

import { listInsights, type GetInsightsDeps } from "../get-insights.ts";
...
/** The same shape apps/server/src/composition/insights.ts builds, restated
 * here since this test has no access to apps/server's composition. No
 * repertoire fixture exists in this suite (below), so the adherence source
 * has nothing to say — a trivial stub is honest about that, rather than
 * wiring the full `@velachess/repertoires` adapter chain to return `[]`
 * anyway. The wiring itself is exercised by apps/server's own insights
 * test, over the real HTTP route. */
const deps: GetInsightsDeps = {
  listRepertoiresWithAdherence: async () => [],
  fetchInsightGameRows: (id) => listGamesForInsights(harness.db, id),
};

describe("listInsights over every source", () => {
  it("returns heterogeneous findings, globally ranked", async () => {
    const report = await listInsights(deps, userId);
