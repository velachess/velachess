 ...
 */
import type { Database, StoredGradedPly } from "@velachess/infra-db";
import {
  createRepertoire,
  createUser,
  games,
...
import { seedsFor, triageAndSeed } from "@velachess/drills";

import { drillSummaryFor, type DrillSummaryDeps } from "../drill-summary.ts";
...
function deps(): DrillSummaryDeps {
  return {
    userIdForGame: (gameId) => userIdForGame(db, gameId),
    listEngineDrillCandidates: (uid, scope) => listEngineDrillCandidates(db, uid, scope),
    seedsFor,
  };
}
...
describe("drillSummaryFor", () => {
  it("counts the drillable plies before anything has been seeded", async () => {
    ...
    expect(await drillSummaryFor(deps(), gameId)).toEqual({
      eligible: 1,
      seeded: 0,
  ...
  it("leaves the opponent's blunders out of the count", async () => {
    ...
    expect((await drillSummaryFor(deps(), gameId)).eligible).toBe(1);
  });
  ...
  it("offers nothing for an imported game nobody was named in", async () => {
    ...
    expect(await drillSummaryFor(deps(), imported!.id)).toEqual({
      eligible: 0,
      seeded: 0,
      triaged: true,
    });
  ...
  it("has nothing to offer before the game is analyzed", async () => {
    ...
    expect(await drillSummaryFor(deps(), unanalyzed!.id)).toEqual({
      eligible: 0,
      seeded: 0,
      triaged: true,
    });
