 ...
 *
 * Module-level pure policy (no DB/queue dependency of its own), reused
 * across modules: repertoires' own slices, drills' seed-lines, and
 * games/judge-games (`dispatch.ts`/`deviation.ts`) all build a tree from a
 * parsed PGN, and every one of them reaches it through this module's
 * `index.ts`.
 */
