...
type GetRepertoireWithChapters = (
  userId: string,
  repertoireId: string,
) => Promise<(Repertoire & { chapters: RepertoireChapter[] }) | null>;
type CountJudgmentsByType = (
  repertoireId: string,
) => Promise<{ type: string; n: number }[]>;
type CountJudgmentsByChapter = (
  repertoireId: string,
) => Promise<{ chapterId: string | null; type: string; n: number }[]>;
type ListUnmatchedGames = (
  repertoireId: string,
) => Promise<{ openingName: string | null; openingUrl: string | null }[]>;
type GetJudgmentRows = (repertoireId: string) => Promise<JudgmentRow[]>;
type CountTrainingByChapter = (
  userId: string,
  repertoireId: string,
) => Promise<ChapterTrainingCounts[]>;

export interface GetRepertoireDeps {
  getRepertoireWithChapters: GetRepertoireWithChapters;
  countJudgmentsByType: CountJudgmentsByType;
  countJudgmentsByChapter: CountJudgmentsByChapter;
  listUnmatchedGames: ListUnmatchedGames;
  getJudgmentRows: GetJudgmentRows;
  countTrainingByChapter: CountTrainingByChapter;
}

export async function getRepertoireDetail(
  deps: GetRepertoireDeps,
  userId: string,
  repertoireId: string,
) {
  ...
  const [byType, byChapterRows, unmatched, judgmentRows, trainingRows] =
    await Promise.all([
      deps.countJudgmentsByType(repertoireId),
      deps.countJudgmentsByChapter(repertoireId),
      deps.listUnmatchedGames(repertoireId),
      deps.getJudgmentRows(repertoireId),
      deps.countTrainingByChapter(userId, repertoireId),
    ]);
