...
import { getChapterForUser } from "@velachess/db";
import type {
  ChapterLineView,
  ChapterStartView,
  IllegalRepertoireMove,
} from "@velachess/repertoire";
import { buildRepertoire, chapterView } from "@velachess/repertoire";
...
export async function getChapterDetail(
  ...
  const row = await getChapterForUser(db, userId, repertoireId, chapterId);
