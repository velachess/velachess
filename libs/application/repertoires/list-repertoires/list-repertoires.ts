 ...
 * was actually played. Thin composition — the math lives in
 * @velachess/repertoire (adherenceMetrics); this file only loads and
 * shapes.
 *
...
import type { Database, Repertoire } from "@velachess/db";
import {
...
export async function listRepertoiresWithAdherence(
  ...
  for (const repertoire of await listRepertoiresByUser(db, userId)) {
    // Small, per-user list — not a hot path worth parallelizing.
    // oxlint-disable-next-line eslint/no-await-in-loop
    const [rows, chapterCount, byType, queue] = await Promise.all([
      getJudgmentRows(db, repertoire.id),
      countChaptersByRepertoire(db, repertoire.id),
      countJudgmentsByType(db, repertoire.id),
      countDrillQueue(db, userId, new Date(), { repertoireId: repertoire.id }),
    ]);
    results.push({
      ...repertoire,
