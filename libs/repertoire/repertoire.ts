/**
 * Single entry point for turning a parsed PGN into everything `findDeviation`
 * needs. `index` is always derived from `tree` — bundling them stops a
 * caller from passing a tree and an index that don't actually match.
 */
