 ...
 *
 * Kept Database-first (not a narrow declared-deps slice) as a deliberate,
 * permanent choice, like `@velachess/repertoires`' `ensureCandidateRepertoires`:
 * six different composition roots across five other modules
 * (accounts/sync-account, games/{import-pgn,judge-games},
 * analysis/{process-analysis,get-analysis}, repertoires/{add-chapter,
 * extract-repertoire}) each wire `triageAndSeed` directly with the
 * `Database` they already hold, adapting it into their own slice's
 * narrow dependency type at the composition root — the same pattern as
 * any other infra query function, not migration debt.
 */
