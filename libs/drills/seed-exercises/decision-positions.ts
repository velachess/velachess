 ...
 *
 * Moved from `libs/repertoire` — this drill slice is its only consumer
 * (`seed-lines.ts`); `buildRepertoire` and the tree/index types it needs
 * are imported from `@velachess/repertoires`'s `index.ts` — the
 * repertoires -> drills cycle that used to force a deep import into the
 * old `@velachess/repertoire` package is gone now that
 * `ensureCandidateRepertoires` takes its drills dependency injected
 * instead of importing it directly (see extract-repertoire.ts's doc
 * comment), so this is a plain one-directional module dependency.
 */

import { isChildNode } from "@velachess/chess";
import type { BuiltRepertoire, RepertoireTree } from "@velachess/repertoires";
